...
var Transmissions_1 = require("./src/Transmissions");
...
casper.each(settings.cities, function (self, city) {
    casper.each(settings.cars, function (self, car) {
        ...
        casper.each(car.models, function (self, carModel) {
            var tr = car.transmissions[0];
            tr = Transmissions_1.Transmissions[tr];
            console.log(tr);
            var url = UrlBuilder_1.UrlBuilder.build(city, car, carModel);
            var cars = [];
    ...
    casper.thenOpen(url, function () {
        ...
        casper.waitForSelector('.catalog_table', function () {
            ...
            if (this.exists('.js-pagination-next')) {
                var href = UrlBuilder_1.UrlBuilder.getBaseUrl() + casper.getElementAttribute('.js-pagination-next', 'href');
